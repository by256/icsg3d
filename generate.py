""""
ICSG3D/generate.py
Script for generating new crystals from a base compound
"""
import argparse
import json
import os
...
from viz import (animate, expand_coordinates, explode, plot_points_3d, resize,
                 tsne_latent, viz)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base', metavar='base', type=str, help='Base Compound', default='LaFeO3')
    parser.add_argument('--batch_size', metavar='batch_size', type=int, help='Batch size', default=100)
    parser.add_argument('--nsamples', metavar='nsamples', type=int, help='Number of samples', default=100)
    parser.add_argument('--var', metavar='var', type=float, help='Variance of sampling', default=0.5)
    parser.add_argument('--eps_frac', metavar='eps_frac', type=float, help='Eps of lattice vector', default=0.25)
    parser.add_argument('--clus_iters', metavar='clus_iters', type=int, help='Iterations of Clustering', default=5)
    ...
    properties = ['formation_energy_per_atom', 'bulk_modulus', 'dielectric_constant', 'eij_max', 'refractive_index', 'shear_modulus', 'energy_per_atom', 'bandgap']
    ...
    if base_compound.startswith('mp-'):
        ...
        base_formula = df[df['pretty_formula'] == base_compound]['pretty_formula'].values[0]
    ...
    S_base = np.load(os.path.join(path, 'species_matrices', base_compound + '.npy')).reshape(1,32,32,32,1)
    C_base = np.load(os.path.join(path, 'coordinate_grids', base_compound + '.npy')).reshape(1,32,32,32,3)
    ...
    for batch in range(int(n_samples/batch_size)):
        ...
        C_prime_samples = M_prime_samples[:,:,:,:,1:].reshape(batch_size, 32, 32,32,3)
        ...
        S_prime_samples = np.argmax(S_prime_samples, axis=-1).reshape(batch_size, 32,32,32,1)
        ...
        for i in range(batch_size):
            # Get coordinates
            try:
                species_sample, mu_samples = watershed_clustering(M_prime_samples[i,:,:,:,0], S_prime_samples[i], S_b_prime_samples[i], max_iters=clustering_max_iters)
                mu_samples = mu_samples*dv_pred[i] - (eps * l_prime_samples[i]) + (dv_pred[i]/2.) 
                coords = np.concatenate([species_sample.reshape((len(species_sample), 1)), mu_samples], axis=-1)
            except Exception:
                print("Failed")
                continue

            # Write to cifs
            structure = to_pymatgen_structure(l_prime_samples[i], coords, alpha, beta, gamma)
            ...
            if not structure.is_valid() or not comp.anonymized_formula in desired_structure:
                continue
            ...
            # Predict properties
            rd = {'formula': formula, 'id': formula_count, 'training': 0, 'target': base_target_value, 'anon_formula': comp.anonymized_formula}
            ...
            np.save(os.path.join(out_dir, 'densities', formula_count), M_prime_samples[i])
            np.save(os.path.join(out_dir, 'species', formula_count), S_prime_samples[i])

            print(batch*batch_size + i, formula, structure.is_valid(), mg.Composition(structure.formula).anonymized_formula, rd[namespace.target])
