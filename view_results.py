"""
## Script for visualising results at each stage of the pipeline
## Example:
## >> python3 view_results.py --name perovskite
--------------------------------------------------
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", metavar="batch_size", type=int, help="Batch size", default=1
    )
    ...
    for M, t in _generator:
        ...
        try:
            species, mus, R = watershed_clustering(
                M[:, :, :, :, 0], S, S_b, return_ws=True, max_iters=5
            )
        except Exception:
            continue
        ...
        mse = mean_squared_error(
            M.reshape(32 * 32 * 32, 4), M_recon.reshape(32 * 32 * 32, 4)
        )
        ...
        try:
            species_pred, mus_pred, R_pred = watershed_clustering(
                M_recon[:, :, :, :, 0], S_recon, S_b_recon, max_iters=5, return_ws=True
            )
        except Exception:
            continue
        coords_pred = np.concatenate(
            [species_pred.reshape((len(species_pred), 1)), mus_pred], axis=-1
        )
        ...
        plt.show(block=True)
        inp = input("...")
        # Continue until user requests a stop
        if inp == "n":
            break
