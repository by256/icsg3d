...
def segment_nuclei(binary, species, intensity, wmin=8, it=1, max_iters=5, min_convexity=0.8, verbose=False):
    ...
    seg_classes = np.array([seg_classes[i] for i in range(len(seg_classes)) if seg_counts[i] > 3])
    ...
    for cl in seg_classes:
        ...
        if convexity >= min_convexity:
            max_class = np.max(R)
            R[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]] = np.where(binary_bbox == cl, max_class+1, R[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]])
            continue
        ...
        if int(np.count_nonzero(wss)/wmin) > len(np.unique(wss))-1 and it < max_iters:
            if verbose:
                print("Segmenting again")
            Rp = segment_nuclei(wss, species_bbox, intensity_bbox, it=it+1, verbose=verbose, max_iters=max_iters, min_convexity=min_convexity)
            max_class = np.max(R)
            Rp = Rp + max_class  # sub region with classes relabelled
            Rp[Rp == max_class] = 0
            R[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]] = np.where(Rp != 0, Rp, R[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]])
        else:
            R[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]] = np.where(wss != 0, wss, R[bbox[0]:bbox[3], bbox[1]:bbox[4], bbox[2]:bbox[5]])
