...
def density_matrix(N, z, l, dims=(32,32,32), sigma=0.5, dist=euclidean, label_frac=1.0, eps_frac=0.25):
...
def to_voxel_params(lp, eps=0.25, d=32):
    ...
    dv = np.concatenate([dx.reshape(batch, 1),dy.reshape(batch, 1),dz.reshape(batch, 1)], axis=-1)
    return dv

def random_rotation_3d(M, S, p, rot_angle=90, nrotations=3):
    """ Randomly rotate an image 3 times around an axis.
    ...
    for i in range(nrotations):
        M_rot = scipy.ndimage.interpolation.rotate(M_rot, rot_angle, mode='nearest', axes=rotations[i], reshape=False)
        S_rot = scipy.ndimage.interpolation.rotate(S_rot, rot_angle, mode='nearest', axes=rotations[i], reshape=False)
        p_rot = scipy.ndimage.interpolation.rotate(p_rot, rot_angle, mode='nearest', axes=rotations[i], reshape=False)
...
def to_pymatgen_structure(lattice_params, coords, alpha=90, beta=90, gamma=90):
    ...
    new_lattice = mg.Lattice.from_parameters(*lattice_params, alpha=alpha, beta=beta, gamma=gamma)
