...
class UnetDataGenerator(Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            X[
                i,
            ] = self.create_lattice_meshgrid(ID, channels=self.n_channels)

            # Store class
            s = np.load(os.path.join(self.data_path, "species_matrices", ID)).reshape(
                (*self.dim, 1)
            )
            y[
                i,
    ...
    def create_lattice_meshgrid(self, ID, channels=4):
        ...
        if channels == 1:
            ...
            p = np.load(os.path.join(self.data_path, "coordinate_grids", ID)).reshape(
                1, *self.dim, 3
            )
