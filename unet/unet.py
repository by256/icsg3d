...
class TrainingPlot(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.val_losses[-1] < self.min_val_loss:
            ...
            self.plot_segmentations(
                epoch, name=os.path.join(self.sdir, "segmentations_best.png")
            )
            self.plot_binary(
                epoch, name=os.path.join(self.sdir, "segmentations_binary_best.png")
            )
    ...
    def plot_segmentations(self, epoch, logs={}, name=None):
        ...
        for M, t in self.val_gen:
            ...
            recon_multi = np.argmax(recon_multi, axis=-1).reshape(
                (len(M), 32, 32, 32, 1)
            )
            S = np.argmax(S, axis=-1).reshape((len(M), 32, 32, 32, 1))
            axes1[ax][0].imshow(
                S[0, :, :, 16, 0],
                vmin=np.min(S[0, :, :, 16, 0]),
                vmax=np.max(S[0, :, :, 16, 0]),
            )
            axes1[ax][1].imshow(
                recon_multi[0, :, :, 16, 0],
                vmin=np.min(recon_multi[0, :, :, 16, 0]),
                vmax=np.max(recon_multi[0, :, :, 16, 0]),
            )
    ...
    def plot_binary(self, epoch, logs={}, name=None):
        ...
        for M, t in self.val_gen:
            ...
            axes1[ax][0].imshow(
                B[0, :, :, 16, 0],
                vmin=np.min(B[0, :, :, 16, 0]),
                vmax=np.max(B[0, :, :, 16, 0]),
            )
            axes1[ax][1].imshow(
                recon_b[0, :, :, 16, 0],
                vmin=np.min(recon_b[0, :, :, 16, 0]),
                vmax=np.max(recon_b[0, :, :, 16, 0]),
            )
            axes1[ax][0].set_xticks([])
            axes1[ax][0].set_yticks([])
            axes1[ax][1].set_xticks([])
...
class AtomUnet:
    ...
    def __init__(
        ...
        self.model.compile(
            loss={
                "soft": weighted_categorical_crossentropy(num_classes),
                "sig": "binary_crossentropy",
            },
        ...
        if weights and os.path.exists(weights):
            ...
            self.filepath = (
                "./saved_models/unet_%d_channel_weights.best.hdf5" % input_shape[-1]
            )
    ...
    def train_generator(
        self, train_gen, val_gen, epochs=100, output_dir="output/unet/"
    ):
        print("Training...")
